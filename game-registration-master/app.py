...
def _sb() -> Client:
    # cria o cliente supabase uma vez por sessão do browser, reutiliza nas reruns
    if "sb_client" not in st.session_state:
        try:
            url = st.secrets["SUPABASE_URL"]
            key = st.secrets["SUPABASE_KEY"]
        except (KeyError, FileNotFoundError):
            st.error("⚠️ Configure SUPABASE_URL e SUPABASE_KEY nos Secrets do app.")
            st.stop()
        st.session_state["sb_client"] = create_client(url, key)

    client = st.session_state["sb_client"]

    # restaura a sessão autenticada a cada rerun do streamlit
...
def delete_collection(coll_id: str):
    # o banco já cuida de desvincular os jogos com ON DELETE SET NULL
    _sb().table("collections").delete().eq("id", coll_id).eq("user_id", _uid()).execute()
...
def insert_game(data: dict):
    ...
    _sb().table("games").insert(payload).execute()


def update_game(game_id: str, data: dict):
    _sb().table("games").update(data).eq("id", game_id).eq("user_id", _uid()).execute()


def delete_game(game_id: str):
    _sb().table("games").delete().eq("id", game_id).eq("user_id", _uid()).execute()


def fetch_all(collection_id=None) -> pd.DataFrame:
    q = _sb().table("games").select("*").eq("user_id", _uid()).order("name")
...
def page_auth():
    ...
    with col:
        ...
        with tab_reg:
            ...
            st.caption("Gratuito. Seus jogos ficam salvos na nuvem, acessíveis de qualquer lugar.")
            email_r  = st.text_input("E-mail", key="reg_email", placeholder="seu@email.com")
            ...
            if st.button("✅ Criar Conta", use_container_width=True, key="btn_reg", type="primary"):
                if not email_r.strip() or not senha_r.strip():
                    ...
                    try:
                        res = _sb().auth.sign_up(
                            {"email": email_r.strip(), "password": senha_r}
                        )
                        if res.user and res.session:
                            ...
                            st.success(
                                ...
                                "depois faça login na aba ao lado."
                            )
                        else:
                            st.error("Não foi possível criar a conta.")
                    except Exception:
                        st.error("Erro ao criar conta. O e-mail já pode estar em uso.")
    ...
    if cid is not None:
        ...
        if not sem_pasta.empty:
            with st.expander("➕ Adicionar jogo existente a esta pasta", expanded=False):
                opcoes = {
                    f"{row['name']} ({row['platform']})": row["id"]
                ...
                if st.button("✅ Mover para esta pasta", disabled=not selecionados, use_container_width=True):
                    for nome in selecionados:
                        _sb().table("games").update({"collection_id": cid}).eq("id", opcoes[nome]).execute()
...
def page_settings():
    ...
    st.subheader("👤 Conta")
    st.info(f"Logado como: **{st.session_state['user'].email}**")
    if st.button("🚪 Sair da conta", key="logout_settings"):
        _sb().auth.sign_out()
...
def main():
    ...
    # valida a sessão antes de qualquer coisa (limpa se token expirou)
    _sb()
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Sair", key="logout_sidebar", use_container_width=True):
            _sb().auth.sign_out()
            st.session_state.clear()
            st.rerun()
